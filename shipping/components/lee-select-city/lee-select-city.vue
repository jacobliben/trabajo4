	...
	export default {
		...
		methods: {
			...
			confirmStart(){
				...
				var start_region = start_place.slice(-1)
				
				if (start_region.length ==0){
					start_region = "全国"
				}
				...
				this.currentSelected.forEach((item,index)=>{
					start_place_code.push(item)
				})
				
				console.log (start_place_code, "start_place_code")
				console.log (dataRegion ,"dataRegion")
				let start_region_value
				
				if (start_place_code.length>0){
					var first_level = start_place_code[0]
					...
					if (start_place_code.length>1){
						...
						if (start_place_code.length>2){
							var third_level = start_place_code[2]
